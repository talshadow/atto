...
public:
    SharedMutex()
        : m_read{kUnlockState}
        , m_writeState{kUnlockState}
        , m_write{kUnlockState}
    {}
    ~SharedMutex() = default;
    ...
    void sharedUnlock() noexcept { --m_read; }
...
private:
    ...
    bool tryLockWriter() noexcept
    {
        ...
        for (unsigned counter = 0; (counter < kCountOfTries) && (!retVal); ++counter) {
            ...
            retVal = m_write.compare_exchange_weak(lockState, kLockState, std::memory_order_seq_cst)
                     && (kUnlockState == lockState);
        }
        return retVal;
    }
...
{
public:
    ReaderMutex(SharedMutex& smutex)
        : m_mutex(smutex)
    {}
    ~ReaderMutex() = default;
